...
for (const f of [
  ...
  'data/metrics_054.json',
  'data/metrics_020.json',
  'data/metrics_908.json'
]) {
  try {
    const p = join(BUILD, f);
    const { createHash } = await import('node:crypto');
    results.build_sha256[f] = {
...
async function run(view) {
  ...
  page.on('response', (r) => {
    if (
      firstTileAt === null &&
      (r.url().includes('ORTO_BFA_') || r.url().includes('WMS_ORTOARGAZKIAK')) &&
      r.status() === 200
    ) {
  ...
  return {
    ...
    first_ortho_tile_ms: firstTileAt,
    navigation: nav,
    interaction_year_change_ms: interaction,
    headline_after_change: headline
  };
}
