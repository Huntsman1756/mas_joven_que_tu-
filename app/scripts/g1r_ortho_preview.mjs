 ...
 *   2. layer-order    — ortho-preview < ortho < sel-muni-outline (raster bajo el vector
 *                       que debe seguir visible) + píxeles reales
 ...
 *   6. compare        — comparador (segundo lienzo __mjtMapB): ortho-b-preview bajo ortho-b
...
const layerOrder = (page) =>
  page.evaluate(() => {
    const ix = (m, id) => (m?.getStyle().layers ?? []).map((l) => l.id).indexOf(id);
    const main = window.__mjtMap,
      cmpMap = window.__mjtMapB;
    return {
      preview: ix(main, 'ortho-preview'),
      ortho: ix(main, 'ortho'),
      cmpPreview: ix(cmpMap, 'ortho-b-preview'),
      cmp: ix(cmpMap, 'ortho-b'),
      munis: ix(main, 'munis-fill'),
      cells: ix(main, 'cells-fill'),
      sel: ix(main, 'sel-muni-outline')
    };
  });
...
/* 1 — P5: 0 requests de imagen ortográfica antes del opt-in */
{
  ...
  ok('p5-zero', orthoReqs.length === 0, {
    preClickRequests: orthoReqs.length,
    urls: orthoReqs.slice(0, 5)
  });
  await ctx.close();
}
...
/* 2 — orden de capas tras opt-in + imagen real pintada */
{
  ...
  await page.waitForFunction(
    () => window.__mjtMap?.getSource('ortho-preview') && window.__mjtMap.getLayer('ortho'),
    { timeout: 20000 }
  );
  await page.waitForTimeout(6000);
  const order = await layerOrder(page);
  const diff = diffPct(before, pngPixels(await page.screenshot()));
  // raster bajo el vector que debe seguir visible: el outline municipal
  // (cells-fill va por debajo del raster desde G5 — la ortofoto lo cubre).
  const pass =
    order.preview >= 0 && order.preview < order.ortho && order.ortho < order.sel && diff > 1;
...
/* 3 — tiles lentas (6 s): el preview pinta antes y permanece visible */
{
  ...
  ok(
    'slow-tiles',
    state.layerVisible &&
      (state.srcLoaded === true || state.srcLoaded === null) &&
      state.tilesPending,
    state
  );
...
/* 4 — stale: preview de A (1990) retrasado; opt-in a B (1975) antes de que A resuelva */
{
  ...
  const state = await page.evaluate(() => {
    ...
    return {
      hasLayer: !!m.getLayer('ortho-preview'),
      url: s ? (s.url ?? s._options?.url ?? null) : null
    };
  });
  const pass =
    state.hasLayer &&
   ...
   * historias y deep links; aquí entra por `ortho2=` (contrato público). */
{
  ...
  await page
    .waitForFunction(() => window.__mjtMapB?.getLayer('ortho-b'), { timeout: 30000 })
    .catch(() => null);
