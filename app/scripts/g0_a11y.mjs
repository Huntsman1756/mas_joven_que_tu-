...
report.labels = await page.evaluate(() => {
  ...
  for (const el of document.querySelectorAll('input, select')) {
    ...
    out.push({
      ...
      id: el.id,
      hasLabel: !!lbl,
      text: lbl?.textContent?.trim() ?? null
    });
  }
  return out;
});
...
report.axe_summary = axe.reduce((a, v) => {
  a[v.impact ?? 'unknown'] = (a[v.impact ?? 'unknown'] || 0) + 1;
  return a;
}, {});
...
report.reduced_motion = await page2.evaluate(() => {
  ...
  return {
    matches: m.matches,
    canvasPresent: !!document.querySelector('#map canvas'),
    headline: document.querySelector('.headline')?.textContent?.trim().slice(0, 80)
  };
});
await ctx2.close();
