...
/**
 * Imagen «antes» por defecto del swipe (G16: misma regla en cortina y en
 * el selector): la campaña más cercana al año personal que no sea la
 * «después»; si coincide o no hay año, la inmediatamente anterior a la
...
export function defaultSwipeBefore(
  ...
  const candidates = after ? list.filter((c) => c.year !== after.year) : list;
  if (!candidates.length) return null;
  const nearest = personalYear !== null ? nearestCampaign(candidates, personalYear) : null;
  if (nearest) return nearest;
  const ai = after ? list.findIndex((c) => c === after || c.year === after.year) : -1;
  return (
    (ai > 0 ? list[ai - 1] : null) ??
    list.find((c) => c.year === 1956 && c.source === 'bizkaia') ??
    candidates[0] ??
    null
  );
}
